...
const getAllBooks = (req, h) => {
  ...
  if (name) {
    check = check.filter((book) => book
      .name.toLowerCase().includes(name.toLowerCase()));
  }
  if (reading) {
    check = check.filter((book) => book.reading === !!Number(reading));
  }
  if (finished) {
    check = check.filter((book) => book.finished === !!Number(finished));
  }

  const response = h.response({
    status: 'success',
    data: {
      books: check.map((book) => ({
        id: book.id,
        name: book.name,
